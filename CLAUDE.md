...
Better Search (v4.2.4) replaces the default WordPress search with a FULLTEXT-powered, relevance-ranked search engine. It also tracks popular search queries and displays a search heatmap. Namespace: `WebberZone\Better_Search`. Prefix: `bsearch`. Requires WordPress 6.6+, PHP 7.4+.
...
Note: `package.json` references `./includes/pro/blocks/src/` in lint/format scripts even in the free repo — this path does not exist here and those commands will no-op for that path.
...
### Core Components
- **`includes/class-main.php`** — Singleton (`Main::get_instance()`). Instantiates all subsystems; admin initialized on `init` hook.
...
- **No pro directory** — This free repo has no `includes/pro/` directory. The `Main` class has a `?Pro\Pro $pro` property but it is never set in the free version.
