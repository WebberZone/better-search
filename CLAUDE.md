...
- **`Search_Redirects`** (`class-search-redirects.php`) — Redirects a matched search keyword to a post, page or URL. Hooks `template_redirect` (query resolved, nothing output yet); exact-match rules run before contains-match. `destination_post` wins over `destination_url` with no fallback if unresolved. Off-site redirects add only that host to `allowed_redirect_hosts`; disable via `add_filter( 'bsearch_allow_offsite_redirect', '__return_false' )` (recommended on Multisite). No JS tracker fires on redirect, so the hit is recorded via `Tracker::update_count()`. `?bsearch_no_redirect=1` bypasses redirection for testing. Pro upgrades `destination_post` to a Tom Select lookup (`bsearch_settings_redirects` filter).
...
- **`Admin`** (`class-admin.php`) — Pro admin additions. Dashboard chart drill-down: click a bar to view top searches for that day (`bsearch_get_day_searches` AJAX, enqueues `chart-interactions.js`). Owns the redirect destination post lookup (`bsearch_destination_post_search` AJAX, admin-only, nonce + `manage_options`). Reuses the Settings API's Tom Select assets via a separate localised object (`window.BSEARCHREDIRECTSTomSelectSettings`, `data-wp-prefix="BSEARCHREDIRECTS"`) instead of the shared one, so saved post IDs preload with titles.
