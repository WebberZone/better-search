...
class Media_Handler {
	...
	public static function get_the_post_thumbnail( $args = array() ) {
		...
		$defaults = array(
			...
			'thumb_html'         => 'html',
			'thumb_default'      => call_user_func( $get_option_callback, 'thumb_default', self::$default_thumb_url ),
	...
	protected static function ensure_loading_and_decoding_attrs( array $attr ): array {
		if ( empty( $attr['loading'] ) ) {
			/**
			 * Filters the default loading attribute applied to thumbnails.
			 *
		...
		if ( empty( $attr['decoding'] ) ) {
			/**
			 * Filters the default decoding attribute applied to thumbnails.
			 *
