...
function get_bsearch_heatmap( $args = array() ) {
	...
	foreach ( $results as $key => $result ) {
		...
		$formatted_count = sprintf( translate_nooped_plural( $translate_nooped_plural, $count ), bsearch_number_format_i18n( $count ) );
		...
		$a[] = sprintf(
			...
			$searchvar,
			$args['show_count'] ? '<span class="bsearch_heatmap_link_count"> (' . bsearch_number_format_i18n( $count ) . ')</span>' : '',
			$args['after_term']
		);
	}
